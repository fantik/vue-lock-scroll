...
class ScrollLock {
  ...
  preventDefault(evt) {
    const e = evt || window.event; // For the case whereby consumers adds a touchmove event listener to document.
  ...
  async restoreOverflowSetting() {
    ...
    await setAsyncTimeout(() => {
      ...
      if (this.previousBodyOverflowSetting !== undefined) {
        document.body.style.overflow = this.previousBodyOverflowSetting; // Restore previousBodyOverflowSetting to undefined
